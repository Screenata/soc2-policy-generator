...
# SOC 2 evidence collection for GitHub
...
dep_count=$(echo "$dep_alerts" | jq 'if type == "array" then length else 0 end')
echo "| Dependabot alerts (open) | **${dep_count}+** | GitHub | \`/dependabot/alerts\` | ${dep_count}+ open dependency alerts |" >> "$OUT"

# Code scanning alerts
code_alerts=$(gh api "repos/${REPO}/code-scanning/alerts?state=open&per_page=1" 2>/dev/null || echo "[]")
code_count=$(echo "$code_alerts" | jq 'if type == "array" then length else 0 end')
